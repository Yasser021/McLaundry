...
@section('konten')
    ...
    <div class="card">
        <div class="card-body">
            ...
            {{-- end alert --}}
            {{-- Form tambah --}}
            <form action="{{ url('outlet/'.$data->id_outlet) }}" method="post">
                @csrf
                @method('PUT')
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="nm_outlet" id="nm" class="form-control"
                        placeholder="Isi Nama Outlet" value="{{ $data->nm_outlet }}">
                </div>
                <div class="mb-3">
                    <label for="alamat" class="mb-2">Alamat</label>
                    <input type="text" name="alamat_outlet" id="alamat" class="form-control"
                        placeholder="Isi Alamat Outlet" value="{{ $data->alamat_outlet }}">
                </div>
                <div class="mb-3">
                    <label for="no" class="mb-2">No.Telp</label>
                    <input type="number" name="no_outlet" id="no" class="form-control"
                        placeholder="Isi Nomor Telepon Outlet" value="{{ $data->no_outlet }}">
                </div>
                <div class="">
                    <button class="btn btn-success" type="submit">Tambah</button>
                    <a href="{{ url('outlet') }}" class="btn btn-secondary mx-2" type="button">Kembali</a>
